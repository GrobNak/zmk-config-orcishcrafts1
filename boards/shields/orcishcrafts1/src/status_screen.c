...
/* Force the function to register as a globally visible symbol */
lv_obj_t *zmk_display_status_screen(void) {
    // 1. Initialize a core, clean canvas container screen object
    lv_obj_t *screen = lv_obj_create(NULL);
    
    // 2. Set strict viewport dimensions to anchor the physical 172x320 screen boundaries
    ...
    // 4. Force immediate graphic cache synchronization 
    lv_scr_load(screen);
